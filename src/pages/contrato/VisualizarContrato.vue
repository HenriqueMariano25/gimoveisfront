<template>
  <v-row no-gutters>
    <v-col>
      <v-row no-gutters>
        <v-col>
          <barra-topo-busca titulo="Contratos" :busca="busca" @buscar="busca = $event"></barra-topo-busca>
        </v-col>
      </v-row>
      <v-row class="mt-0">
        <v-col>
          <v-card class="border-radius pa-3">
            ...
            >
              <template v-slot:expanded-item="{ headers, item }">
                <td :colspan="headers.length" style="background-color: #d5e6fd">
                  <ul class="pa-3" style="list-style-type: none;">
                    <li><span><strong>Data de ínicio: </strong>{{
                    ...
                    <li v-if="item.fiadores">
                      <span v-if="item.fiadores[0] !== null"><strong>Fiadores:</strong></span>
                    ...
                    <li v-if="item.carencia"><span><strong>Carência: </strong>{{ item.carencia }}</span></li>
                    <li><span><strong>Vigência: </strong>{{ $dayjs(item.data_fim).diff(item.data_inicio, 'month') }} meses</span>
                    </li>
                  </ul>
                </td>
              </template>
              <template v-slot:item="{ item }">
                <tr>
                  <td @click.prevent="abrirDetalhes(item, $event)">{{ ("0000" + item.id).slice(-4) }}</td>
                  <td @click.prevent="abrirDetalhes(item, $event)">{{ item.nome_imovel }}</td>
                  <td @click.prevent="abrirDetalhes(item, $event)">{{ item.nome_responsavel }}</td>
                  <td @click.prevent="abrirDetalhes(item, $event)">{{ item.nome_cliente }}</td>
                  <td @click.prevent="abrirDetalhes(item, $event)"><span :class="{'falta-pdf': !item.nome_pdf}"
                                                                         class="text-capitalize">{{
                      !item.nome_pdf ? 'Falta PDF' : item.status
                    }}</span></td>
                    ...
                    <v-tooltip top>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn icon v-bind="attrs" v-on="on" color="black"
                               @click="dialogContrato = true; idContrato = item.id">
                          <v-icon dark>
                            mdi-pencil
                          </v-icon>
                        </v-btn>
                    ...
                    <v-tooltip top>
                      <template v-slot:activator="{ on, attrs }">
                        <v-btn
                            ...
                            v-on="on"
                            color="black"
                            @click.prevent="dialogDeletar = true; contrato = item"
                        >
                          <v-icon dark>
                            mdi-delete
                          </v-icon>
    ...
    <dialog-deletar
        :texto="`Certeza que deseja deletar o contrato: ${ contrato ? ('0000' + contrato.id).slice(-4) : ''} ?`"
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: '', value: 'acoes', align: 'center', sortable: false, width: '170px'},
      ],
      expanded: [],
