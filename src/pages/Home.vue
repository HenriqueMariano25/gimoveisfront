...
<template>
  <v-row>
          ...
          <v-tabs-items v-model="tab" touchless>
            ...
            <v-tab-item>
              <v-row class="mt-1" no-gutters>
                <v-col>
                  <v-row no-gutters class="pl-2 pr-2" justify="space-between" align="center">
                    <v-col cols="auto">
                    ...
                    <v-col cols="auto">
                      <v-btn depressed :color="'var(--preto-principal)'" dark @click="gerarRelatorioContratosVencendo()"
                             :fab="$isMobile">
                    ...
                    <v-col>
                      ...
                      >
                        <template v-slot:item="{ item }">
                          <tr>
                            <td @dblclick.prevent="dialogContrato = true; idContrato = item.id; editandoContratoVencido = true">
              ...
              <v-row class="mt-1" no-gutters>
                <v-col>
                  <v-row no-gutters class="pl-2 pr-2" justify="space-between" align="center">
                    ...
                    <v-col cols="auto">
                      <v-btn depressed :color="'var(--preto-principal)'" dark @click="gerarRelatorioBoletosVencidos()"
                             :fab="$isMobile">
                        <v-icon :class="{ 'mr-2': !$isMobile}">
                          mdi-printer
                        </v-icon>
                  ...
                  <v-row class="mt-0">
                    <v-col>
                      ...
                      >
                        <template v-slot:item="{ item }">
                          <tr>
                            <td @dblclick.prevent="dialogBoleto = true; dadosBoleto = item">
                              {{ ("000000" + item.id).slice(-6) }}
                            </td>
                            <td @dblclick.prevent="dialogBoleto = true; dadosBoleto = item">{{ item.imovel_nome }}</td>
                            <td @dblclick.prevent="dialogBoleto = true; dadosBoleto = item">{{ item.cliente_nome }}</td>
                            <td @dblclick.prevent="dialogBoleto = true; dadosBoleto = item">
              ...
              <v-row class="mt-1" no-gutters>
                <v-col>
                  <v-row no-gutters class="pl-2 pr-2" justify="space-between" align="center">
                    ...
                    <v-col cols="auto">
                      <v-btn depressed :color="'var(--preto-principal)'" dark @click="gerarRelatorioContratosReajustar()"
                             :fab="$isMobile">
                        <v-icon :class="{ 'mr-2': !$isMobile}">
                          mdi-printer
                        </v-icon>
                  ...
                  <v-row class="mt-0">
                    <v-col>
                      ...
                      >
                        <template v-slot:item="{ item }">
                          <tr>
                            <td @dblclick.prevent="dialogContrato = true; idContrato = item.id; editandoContratoVencido = false">
                              {{ ("0000" + item.id).slice(-4) }}
                            </td>
                            <td @dblclick.prevent="dialogContrato = true; idContrato = item.id; editandoContratoVencido = false">
                              {{ item.imovel_nome }}
    ...
    <dialog-boleto
        ...
        :dados="dadosBoleto"
        @cancelar="dialogBoleto = false; dadosBoleto = null"
    ...
    <dialog-contrato
        ...
        @cancelar="dialogContrato = false; idContrato = ''"
...
export default {
  ...
  methods: {
    ...
    gerarRelatorioContratosVencendo() {
      ...
      doc.setFontSize(14)
      doc.text(`Total: ${this.contratosVencendo.length}`, 200, 21, null, null, "right")
      doc.autoTable({
      ...
      window.open(doc.output('bloburl', {filename: 'tabela_contratos_para_vencer.pdf'}));
    },
    ...
    gerarRelatorioBoletosVencidos() {
      ...
      doc.setFontSize(14)
      doc.text(`Total: ${this.boletosVencendo.length}`, 200, 21, null, null, "right")
      doc.autoTable({
      ...
      window.open(doc.output('bloburl', {filename: 'tabela_boletos_vencidos.pdf'}));
    },
    ...
    gerarRelatorioContratosReajustar() {
      ...
      doc.setFontSize(14)
      doc.text(`Total: ${this.contratosParaReajustar.length}`, 200, 21, null, null, "right")
      doc.autoTable({
      ...
      window.open(doc.output('bloburl', {filename: 'tabela_contratos_para_reajustar.pdf'}));
    },
